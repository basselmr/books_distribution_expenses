    ...
    if category_id==None:
        ...
        if request.method == 'POST':
            ...
            if request.POST.get('method') == 'delete':
                ...
                new_value = request.POST.get('updateValue')  # Get the new value from the form
...
def publishers(request,publisher_id=None):
        if request.user.is_anonymous:
            return redirect('/login')
        if publisher_id==None:
            if request.method=='GET':
                #for key, value in request.GET.items():
                #    print(f'{key}: {value}')
                publisherList = publisherModel.objects.all()
                if request.GET.get('publisher'):
                    search = request.GET['publisher']
                    publisherList = publisherList.filter(publisher__icontains=search)           
                context = {
                ...
                try:
                    ...
                    # Parse JSON data from the request
                    data = json.loads(request.body)
                    # Check if the publisher already exists in JSON data
                    publisher_name = data.get('publisher')
                    if not publisher_name:
                        return JsonResponse({"error": "publisher field required"}, status=400,content_type="application/json")
                    if publisherModel.objects.filter(publisher=publisher_name).exists():
                        return JsonResponse({"error": "Publisher already exists"}, status=400,content_type="application/json")
                    # Create an instance of Category model
                    new_publisher = publisherModel(publisher=publisher_name)
                    # Save the new record to the database
                    new_publisher.save()
                    publisherList = publisherModel.objects.all()
                    context = {
                        'publisherList': publisherList,
                        'success':'New publisher added'
                    }
                    return render(request,'publishers.html',context,status=201)
                    #return JsonResponse({"message": "New record added"}, status=201,content_type="application/json")
                except json.JSONDecodeError as e:
                    return JsonResponse({"error": "Invalid JSON data"}, status=400,content_type="application/json")
                except IntegrityError as e:
            ...
            if request.method == 'POST':
                ...
                if request.POST.get('method') == 'delete':
                    # Retrieve the record to be deleted
                    publisher = publisherModel.objects.get(pk=publisher_id)
                    # Delete the record
                    publisher.delete()
                # Redirect back to the same page
                    ...
                    # Update the record attributes
                    new_value = request.POST.get('updateValue')  # Get the new value from the form
                    publisher.publisher = new_value  # Update the attribute value
                    publisher.save()  # Save the changes
                    # Redirect back to the same page or any desired page
                    return redirect("/publishers")
        return JsonResponse({"error": "Method Not Allowed"}, status=405)
